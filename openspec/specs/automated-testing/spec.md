...
Provide CI-safe automated test layers that verify the TypeScript bridge
contract, the MCP protocol path, and the Swift logic (models, validation,
recurring-event slot matching) without requiring calendar access or TCC
permission, so regressions in the riskiest code are caught in CI rather than
only by manually running the CLI against a real calendar.
